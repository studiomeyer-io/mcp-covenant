...
//!
//! Adapted from the proven client core of [`mcp-gauntlet`](https://github.com/studiomeyer-io/mcp-gauntlet)
//! (same author, MIT). `mcp-covenant` only reads from a server — it lists tools,
//! resources and prompts and never calls a tool — so this is the read-only slice.
...
use crate::protocol::{
    InitializeResult, JsonRpcResponse, ListPromptsResult, ListResourcesResult, ListToolsResult,
    Prompt, Resource, RpcError, Tool, JSONRPC_VERSION, LATEST_PROTOCOL_VERSION,
};
...
impl McpClient {
    ...
    pub async fn connect_stdio(
        ...
        // hostile server that never sends a newline can't drive us to OOM.
        {
    ...
    pub async fn connect_http(url: &str, default_timeout: Duration) -> Result<McpClient, Error> {
        let client = reqwest::Client::builder()
            ...
            .user_agent(concat!("mcp-covenant/", env!("CARGO_PKG_VERSION")))
            .build()
    ...
    pub async fn initialize(&self) -> Result<InitializeResult, Error> {
        let params = json!({
            ...
            "capabilities": {},
            "clientInfo": {"name": "mcp-covenant", "version": env!("CARGO_PKG_VERSION")}
        });
    ...
    /// List the server's resources (`resources/list`). Returns an empty vec if the
    /// server does not advertise the resources capability (method not found).
    pub async fn list_resources(&self) -> Result<Vec<Resource>, Error> {
        match self.request("resources/list", json!({})).await {
            Ok(res) => {
                let lr: ListResourcesResult = serde_json::from_value(res)?;
                Ok(lr.resources)
            }
            Err(Error::Rpc(e)) if e.code == -32601 => Ok(Vec::new()),
            Err(e) => Err(e),
        }
    }

    /// List the server's prompts (`prompts/list`). Returns an empty vec if the server
    /// does not advertise the prompts capability (method not found).
    pub async fn list_prompts(&self) -> Result<Vec<Prompt>, Error> {
        match self.request("prompts/list", json!({})).await {
            Ok(res) => {
                let lp: ListPromptsResult = serde_json::from_value(res)?;
                Ok(lp.prompts)
            }
            Err(Error::Rpc(e)) if e.code == -32601 => Ok(Vec::new()),
            Err(e) => Err(e),
...
/// Parse one newline-delimited JSON-RPC line and deliver it to the waiting caller.
        ...
        for line in text.lines() {
            ...
            if let Some(rest) = line.strip_prefix("data:") {
                ...
                if let Ok(resp) = serde_json::from_str::<JsonRpcResponse>(rest) {
                    if resp.id.as_ref().and_then(value_as_u64) == Some(id) {
